...
hashtags = ["#HaddadÉLula"]


for hashtag in hashtags:
  bots_file = open(hashtag+"_bots", "w+")

  for tweet in tweepy.Cursor(api.search,q=hashtag,count=1,lang="pt", tweet_mode='extended').items():
    print "\n****\n"
    user_score = 0
    rtuser_score = 0
    if 'retweeted_status' in dir(tweet):
      rtuser_id = tweet.retweeted_status.user.id
      rtuser_score = data.bot_score(rtuser_id, bom)
      if(rtuser_score > 0.7):
        bots_file.write("{},{}".format(rtuser_id, rtuser_score))
    else:
      user_id = tweet.user.id
      user_score = data.bot_score(user_id, bom)
      if(user_score > 0.7):
        bots_file.write("{},{}".format(user_id, user_score)) 
